<x-app-layout>
        ...
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="card">
                        <div class="card-body">
                            <form action="{{ route('updateblog', $edit_blog->id) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="form-group">
                                    <label for="title">Blog Title</label>
                                    <input type="text" name="title" value="{{ $edit_blog->title }}" class="form-control" id="title" aria-describedby="emailHelp">
                                    @error('title')
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="file" name="blog_img" value="{{ $edit_blog->blog_img }}" class="form-control" id="file" aria-describedby="emailHelp">
                                    <img src="{{ asset($edit_blog->blog_img) }}" alt="" width="50" height="50">
                                    @error('blog_img')
                                    <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="description" id="description" class="form-control" cols="100" rows="3">{{ $edit_blog->description }}</textarea>
                                    @error('description')
                                    <span class="text-danger">{{ $message }}</span>
                                    @enderror
    ...
    <script type="text/javascript">
        var tagDetails = {!! json_encode($edit_blog->blog) !!};
        $(document).ready(function() {
            tagDetails.forEach((index, value)=> {
                console.log('value',index.tag_name);
                var tag = '<div id="group" style="display: flex; margin-top:7px"><input type="text" style="margin-right:2px;" id="delete" name="tag[]" value="'+index.tag_name+'" class="form-control" aria-describedby="emailHelp"><br><a href="Javascript:void(0)" id="remove" class="btn btn-danger removetag">-</a></div>';
                $('.new').append(tag);
            });
        });
        $(document).on('click', '.addtag', function() {
            var newtag = '<div id="group" style="display: flex; margin-top:7px"><input type="text" style="margin-right:2px;" id="delete" name="tag[]" value="" class="form-control" aria-describedby="emailHelp"><br><a href="Javascript:void(0)" id="remove" class="btn btn-danger removetag">-</a><br></div>';
            $('.new').append(newtag);
        });
